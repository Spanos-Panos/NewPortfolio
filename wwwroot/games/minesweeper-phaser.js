// Premium Minesweeper Game (Phaser.js)
(function(){
  if (!window.Phaser) {
  ...
  function preload() {
    ...
    this.load.image('tile', 'https://assets.codepen.io/1468070/minesweeper-tile-2.5d.png');
    this.load.image('mine', 'https://assets.codepen.io/1468070/mine-2.5d.png');
    this.load.audio('reveal', 'https://cdn.pixabay.com/audio/2022/07/26/audio_124bfae5e2.mp3');
    this.load.audio('boom', 'https://cdn.pixabay.com/audio/2022/07/26/audio_124bfae5e2.mp3');
  }
  function create() {
    ...
    const title = this.add.text(0,-40,'MINESWEEPER',{fontFamily:'Arial Black',fontSize:'2.2em',color:'#6cf'}).setOrigin(0.5);
  ...
  function startGame(){
    started = true;
    // ...gameplay logic (minesweeper grid, reveal, win/lose, etc.)
    this.add.text(width/2,height/2,'[Minesweeper Here]',{fontFamily:'Arial Black',fontSize:'1.5em',color:'#fff'}).setOrigin(0.5);
  }
  ...
  if(window.__minesweeperPhaserGame && window.__minesweeperPhaserGame.destroy) window.__minesweeperPhaserGame.destroy(true);
  game = new Phaser.Game(config);
  window.__minesweeperPhaserGame = game;
});
