...
(function(){
  ...
  function preload() {
    ...
    this.load.image('block', 'https://assets.codepen.io/1468070/tetris-block-2.5d.png');
    this.load.audio('move', 'https://cdn.pixabay.com/audio/2022/07/26/audio_124bfae5e2.mp3');
    this.load.audio('clear', 'https://cdn.pixabay.com/audio/2022/07/26/audio_124bfae5e2.mp3');
  }
  function create() {
    ...
    const title = this.add.text(0,-40,'TETRIS',{fontFamily:'Arial Black',fontSize:'2.2em',color:'#6cf'}).setOrigin(0.5);
  ...
  function startGame(){
    started = true;
    // ...gameplay logic (tetris grid, blocks, movement, clear, score, etc.)
    this.add.text(width/2,height/2,'[Tetris Game Here]',{fontFamily:'Arial Black',fontSize:'1.5em',color:'#fff'}).setOrigin(0.5);
  }
  ...
  if(window.__tetrisPhaserGame && window.__tetrisPhaserGame.destroy) window.__tetrisPhaserGame.destroy(true);
  game = new Phaser.Game(config);
  window.__tetrisPhaserGame = game;
});
